...
- **changelog** - update `CHANGELOG.md` with entries for the changes.
  ...
  files under `browserchooser/`, `src/`,
...
For example, if a commit contains `browserchooser/src/main.cpp`, and `CHANGELOG.md`, the component
...
- Header file `include/browserchooser/util.h` → `util:`.
